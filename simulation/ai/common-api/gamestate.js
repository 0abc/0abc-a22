...
m.GameState.prototype.init = function(SharedScript, state, player) {
  ...
  this.phases = [{ name: "phase_village" }, { name: "phase_town" }, { name: "phase_city" }, { name: "phase_metropolis" }];
  let cctemplate = this.getTemplate(this.applyCiv("{civ}/structure_civil_centre"));
  if (!cctemplate)
    return;
...
{
  let shipMechantTemplateName = this.applyCiv("{civ}/ship_merchant");
  let supportTraderTemplateName = this.applyCiv("{civ}/support_trader");
