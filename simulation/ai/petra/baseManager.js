...
m.BaseManager.prototype.findBestDropsiteLocation = function(gameState, resource)
{
  ...
  let template = gameState.getTemplate(gameState.applyCiv("{civ}/structure_storehouse"));
  let halfSize = 0;
...
m.BaseManager.prototype.checkResourceLevels = function (gameState, queues)
{
  for (let type of gameState.sharedScript.resourceInfo.codes)
  {
    if (type === "food")
    {
      if (gameState.ai.HQ.canBuild(gameState, "{civ}/structure_field"))  // let's see if we need to add new farms.
      {
        ...
        if (numFarms + numQueue === 0)  // starting game, rely on fruits as long as we have enough of them
        {
          if (count < 600)
          {
            queues.field.addPlan(new m.ConstructionPlan(gameState, "{civ}/structure_field", { "base": this.ID }));
      ...
      else if (gameState.isTemplateDisabled(gameState.applyCiv("{civ}/structure_field")) &&
         !queues.corral.hasQueuedUnits() &&
         !gameState.getOwnEntitiesByClass("Corral", true).hasEntities() &&
         gameState.ai.HQ.canBuild(gameState, "{civ}/structure_corral"))
      {
        let count = this.getResourceLevel(gameState, type, gameState.currentPhase() > 1);  // animals are not accounted
        if (count < 600)
        {
          queues.corral.addPlan(new m.ConstructionPlan(gameState, "{civ}/structure_corral", { "base": this.ID }));
      ...
      if (gameState.ai.playedTurn > this.gatherers[type].nextCheck)
      {
        ...
        if (total > 150 || (total > 60 && type !== "wood"))
        {
          ...
          if (ratio > 0.15)
          {
            ...
            if (newDP.quality > 50 && gameState.ai.HQ.canBuild(gameState, "{civ}/structure_storehouse"))
              queues.dropsites.addPlan(new m.ConstructionPlan(gameState, "{civ}/structure_storehouse", { "base": this.ID, "type": type }, newDP.pos));
            else if (!gameState.getOwnFoundations().filter(API3.Filters.byClass("CivCentre")).hasEntities() && !queues.civilCentre.hasQueuedUnits())
            {
              ...
              if (!gameState.ai.HQ.buildNewBase(gameState, queues, type) && newDP.quality > Math.min(25, 50*0.15/ratio) &&
                gameState.ai.HQ.canBuild(gameState, "{civ}/structure_storehouse"))
                queues.dropsites.addPlan(new m.ConstructionPlan(gameState, "{civ}/structure_storehouse", { "base": this.ID, "type": type }, newDP.pos));
            }
          }
