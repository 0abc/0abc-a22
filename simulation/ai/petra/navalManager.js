...
m.NavalManager.prototype.buildNavalStructures = function(gameState, queues)
{
  ...
  if (gameState.getPopulation() > this.Config.Economy.popForDock)
  {
    if (queues.dock.countQueuedUnitsWithClass("NavalMarket") === 0 &&
      ...
      gameState.ai.HQ.canBuild(gameState, "{civ}/structure_dock"))
    {
      ...
      for (let base of gameState.ai.HQ.baseManagers)
      {
        ...
        for (let sea of remaining)
        {
          ...
          queues.dock.addPlan(new m.ConstructionPlan(gameState, "{civ}/structure_dock", { "land": wantedLand, "sea": sea }));
          dockStarted = true;
          break;
        }
      }
  ...
  if (!this.docks.filter(API3.Filters.byClass("Dock")).hasEntities() ||
    ...
    return;
  let template;
  if (gameState.ai.HQ.canBuild(gameState, "{civ}/structure_harbour"))
    template = "{civ}/structure_harbour";
  else if (gameState.ai.HQ.canBuild(gameState, "{civ}/structure_shipyard"))
    template = "{civ}/structure_shipyard";
  else
    return;
  let wantedLand = {};
  for (let base of gameState.ai.HQ.baseManagers)
