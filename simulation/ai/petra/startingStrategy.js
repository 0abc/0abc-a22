...
m.HQ.prototype.gameAnalysis = function(gameState)
{
  ...
  {
    let template = gameState.applyCiv("{civ}/structure_civil_centre");
...
m.HQ.prototype.buildFirstBase = function(gameState)
{
  ...
  if (!total.canAfford(new API3.Resources(template.cost())) && !docks.hasEntities())
  {
    ...
    if (gameState.ai.queues.dock.hasQueuedUnits())
      return;
    template = gameState.applyCiv("{civ}/structure_dock");
    if (gameState.isTemplateDisabled(template))
      return;
    template = gameState.getTemplate(template);
  ...
  if (goal === "dock")
  {
    ...
    gameState.ai.queues.dock.addPlan(new m.ConstructionPlan(gameState, "{civ}/structure_dock", { "sea": sea, "proximity": startingPoint[imax].pos }));
  }
  else
    gameState.ai.queues.civilCentre.addPlan(new m.ConstructionPlan(gameState, "{civ}/structure_civil_centre", { "base": -1, "resource": "wood", "proximity": startingPoint[imax].pos }));
};
...
m.HQ.prototype.configFirstBase = function(gameState)
{
  ...
  let template = gameState.applyCiv("{civ}/structure_storehouse");
  ...
  if (this.needCorral)
  {
    template = gameState.applyCiv("{civ}/structure_corral");
